  ...
  where
    helper args stdFuns =
        case args of
            ...
            ("-f":fileName:_) ->
                runInputTBehavior (useFile fileName) defaultSettings $
                Control.Monad.void (fileMode False stdFuns 1)
            ("-vf":fileName:_) ->
                runInputTBehavior (useFile fileName) defaultSettings $
                Control.Monad.void (fileMode True stdFuns 1)
            (fileName:_) ->
                runInputTBehavior (useFile fileName) defaultSettings $
                Control.Monad.void (fileMode False stdFuns 1)
...
replMode verbose sfFuns = do
    ...
    case a of
        ...
        Just (':':'m':_) -> do
            let s = Map.lookup (head $ tail $ words $ fromJust a) sfFuns
            case s of
        ...
        Just (':':'l':_) -> do
            (_, newFuns) <-
                runInputTBehavior (useFile $ head $ tail $ words $ fromJust a) defaultSettings $
                fileMode verbose sfFuns 1
            outputStrLn "done loading!" --TODO (Documentation)
            replMode verbose newFuns
        Just input -> do
            let (out, newFuns) = runState (runLine verbose input 1) sfFuns
            if all isSpace out
                then outputStr ""
...
fileMode verbose sfFuns counter = do
    ...
    case a of
        ...
        Just input -> do
            let (b, c) = runState (runLine verbose input counter) sfFuns
            if length b > 1
